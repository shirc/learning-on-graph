function [Pz_d_final, Pw_z_final, LogL_final, nIter_final] = LTM3(X, K, W, options, Pz_d, Pw_z, trainIdx, trainPz_d)
...
if alpha > 0
    ...
    L = alpha*L;
    dLen = full(sum(X,1));
    OmegaL = (1-lambdaB)*spdiags(dLen',0,speye(size(W,1)))+L+beta*A;
end
        ...
        if nIter > minIter
            if selectInit
                ...
                if alpha > 0
                    ...
                    newLogL = newLogL - 0.5*beta*sum(A*sum((log(Pz_d+ZERO_OFFSET)-log(Qz_d+ZERO_OFFSET)).*(Pz_d-Qz_d))');
                end
